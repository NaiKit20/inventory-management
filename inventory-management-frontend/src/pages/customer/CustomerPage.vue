<template>
  <PageCard title="ข้อมูลลูกค้า">
    <template #actions>
      <button
        @click="showAddModal = true"
        class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-4 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
      >
        เพิ่มลูกค้า
      </button>
    </template>

    <div class="overflow-auto border rounded-lg">
      <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-100 text-xs text-slate-600 uppercase">
          <tr>
            <th class="px-4 py-3 text-left">รหัสลูกค้า</th>
            <th class="px-4 py-3 text-left">ชื่อลูกค้า</th>
            <th class="px-4 py-3 text-center">การจัดการ</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100 text-sm">
          <tr
            v-for="(value, index) in customers"
            :key="index"
            class="hover:bg-slate-50"
          >
            <td class="px-4 py-3 align-top font-medium text-slate-800">
              {{ value && value.cus_id }}
            </td>
            <td class="px-4 py-3 align-top">
              <template v-if="value && editingId === value.cus_id">
                <input
                  v-model="editData.cus_name"
                  type="text"
                  class="border rounded px-3 py-2 w-full"
                  placeholder="เช่น ลูกค้า A"
                />
              </template>
              <template v-else>
                {{ value && value.cus_name }}
              </template>
            </td>
            <td class="px-4 py-3 align-top">
              <div class="flex gap-2 justify-center">
                <template v-if="value && editingId === value.cus_id">
                  <button
                    @click="saveEdit(value)"
                    class="bg-green-600 hover:bg-green-700 text-white text-sm px-3 py-2 rounded"
                  >
                    บันทึก
                  </button>
                  <button
                    @click="cancelEdit"
                    class="bg-gray-300 hover:bg-gray-400 text-gray-900 text-sm px-3 py-2 rounded"
                  >
                    ยกเลิก
                  </button>
                </template>
                <template v-else>
                  <button
                    @click="onEdit(value)"
                    class="bg-blue-600 hover:bg-blue-700 text-white text-sm px-3 py-2 rounded"
                  >
                    แก้ไข
                  </button>
                  <button
                    @click="onDelete(value)"
                    class="bg-red-600 hover:bg-red-700 text-white text-sm px-3 py-2 rounded"
                  >
                    ลบ
                  </button>
                </template>
              </div>
            </td>
          </tr>
          <tr v-if="(customers || []).length === 0">
            <td
              colspan="4"
              class="px-4 py-6 text-center text-slate-500 text-sm"
            >
              ยังไม่มีลูกค้า
            </td>
          </tr>
        </tbody>
      </table>
    </div>
  </PageCard>

  <!-- Modal -->
  <Teleport to="body">
    <transition name="fade">
      <div
        v-if="showAddModal"
        class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-start md:items-center justify-center z-50 overflow-y-auto p-4"
      >
        <div
          class="relative bg-white w-full max-w-lg rounded-xl shadow-xl flex flex-col max-h-[90vh]"
        >
          <div
            class="sticky top-0 bg-white/95 backdrop-blur px-6 py-4 border-b flex items-center justify-between rounded-t-xl"
          >
            <h2 class="text-lg font-semibold text-slate-800">
              เพิ่มลูกค้าใหม่
            </h2>
            <button
              @click="showAddModal = false"
              class="text-slate-500 hover:text-slate-700 transition-colors"
              aria-label="Close"
            >
              ✕
            </button>
          </div>
          <div class="px-6 py-6 overflow-y-auto">
            <form @submit.prevent="onCreate">
              <div class="space-y-4">
                <div>
                  <label class="block text-sm text-gray-600 mb-1"
                    >รหัสลูกค้า *</label
                  >
                  <div class="relative">
                    <input
                      v-model.trim="addData.cus_id"
                      type="text"
                      class="border rounded px-3 py-2 w-full"
                      placeholder="เช่น C004"
                      :class="{ 'border-red-500': addErrors.cus_id }"
                    />
                    <button
                      type="button"
                      @click="generateUserId"
                      class="absolute right-2 top-1/2 -translate-y-1/2 bg-blue-600 hover:bg-blue-700 text-white text-xs px-2 py-1 rounded"
                    >
                      สุ่มรหัส
                    </button>
                  </div>
                  <p v-if="addErrors.cus_id" class="text-red-600 text-xs mt-1">
                    {{ addErrors.cus_id }}
                  </p>
                </div>
                <div>
                  <label class="block text-sm text-gray-600 mb-1"
                    >ชื่อลูกค้า *</label
                  >
                  <input
                    v-model.trim="addData.cus_name"
                    type="text"
                    class="border rounded px-3 py-2 w-full"
                    placeholder="เช่น ลูกค้า D"
                    :class="{ 'border-red-500': addErrors.cus_name }"
                  />
                  <p
                    v-if="addErrors.cus_name"
                    class="text-red-600 text-xs mt-1"
                  >
                    {{ addErrors.cus_name }}
                  </p>
                </div>
              </div>

              <div class="flex justify-end gap-2 mt-6">
                <button
                  type="button"
                  @click="showAddModal = false"
                  class="bg-gray-300 hover:bg-gray-400 text-gray-900 text-sm px-4 py-2 rounded"
                >
                  ยกเลิก
                </button>
                <button
                  type="submit"
                  class="bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 rounded"
                  :disabled="loading"
                >
                  <span v-if="!loading">บันทึก</span>
                  <span v-else>กำลังบันทึก...</span>
                </button>
              </div>
            </form>
          </div>
        </div>
      </div>
    </transition>
  </Teleport>
</template>

<script>
import { mapState } from "vuex";
import PageCard from "@components/PageCard.vue";
import { Teleport } from "vue";

export default {
  name: "customerPage",
  components: { PageCard },
  data() {
    return {
      showAddModal: false,
      editingId: null,
      editData: {
        cus_id: "",
        cus_name: "",
      },
      addData: {
        cus_id: "",
        cus_name: "",
      },
      addErrors: {},
    };
  },
  mounted() {
    this.$store.dispatch("apiService/getAllCustomers");
  },
  watch: {
    showAddModal(newVal) {
      if (!newVal) {
        this.addData = { cus_id: "", cus_name: "" };
        this.addErrors = {};
      }
    },
  },
  computed: {
    ...mapState({
      loading: (state) => state.apiService.loading,
      customers: (state) => state.apiService && state.apiService.customers,
    }),
  },
  methods: {
    validateForm(item) {
      const errors = {};
      if (!item.cus_id || item.cus_id.length !== 5)
        errors.cus_id = "กรุณากรอกรหัสลูกค้าให้ครบ 5 ตำแหน่ง";

      if (!item.cus_name) errors.cus_name = "กรุณากรอกชื่อลูกค้า";

      this.addErrors = errors;
      return Object.keys(errors).length === 0;
    },
    onCreate() {
      if (!this.validateForm(this.addData)) return;
      this.$store
        .dispatch("apiService/insertCustomers", { ...this.addData })
        .then(() => {
          this.$store.dispatch("apiService/getAllCustomers").finally(() => {
            this.showAddModal = false;
          });
        })
        .catch((error) => {
          alert("เกิดข้อผิดพลาดในการเพิ่มลูกค้า: " + error.message);
        });
    },
    onEdit(item) {
      this.editingId = item.cus_id;
      this.editData = {
        cus_id: item.cus_id,
        cus_name: item.cus_name,
      };
    },
    cancelEdit() {
      this.editingId = null;
      this.editData = { cus_id: "", cus_name: "" };
    },
    saveEdit() {
      if (!this.validateForm(this.editData)) return;
      this.$store
        .dispatch("apiService/updateCustomers", { ...this.editData })
        .then(() => {
          this.$store.dispatch("apiService/getAllCustomers").finally(() => {
            this.cancelEdit();
          });
        })
        .catch((error) => {
          alert("เกิดข้อผิดพลาดในการแก้ไขลูกค้า: " + error.message);
        });
    },
    onDelete(item) {
      if (confirm(`ยืนยันการลบลูกค้า ${item.cus_name}?`)) {
        this.$store
          .dispatch("apiService/deleteCustomers", item.cus_id)
          .then(() => {
            this.$store.dispatch("apiService/getAllCustomers");
          })
          .catch((error) => {
            alert("เกิดข้อผิดพลาดในการลบลูกค้า: " + error.message);
          });
      }
    },
    generateUserId() {
      const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
      let result = "";
      for (let i = 0; i < 5; i++) {
        result += chars.charAt(Math.floor(Math.random() * chars.length));
      }
      this.addData.cus_id = result;
    },
  },
};
</script>

<style scoped>
.fade-enter-active,
.fade-leave-active {
  transition: opacity 0.2s ease;
}
.fade-enter-from,
.fade-leave-to {
  opacity: 0;
}
</style>
