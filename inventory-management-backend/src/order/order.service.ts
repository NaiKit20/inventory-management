...
export class OrderService {
  constructor(
    ...
    @InjectRepository(GoodsName) private goodsNameRepository: Repository<GoodsName>,
  ...
  async deleteHeaderOrder(dto: DeleteHeaderOrderDto) {
    ...
    if (!headerOrder) {
      throw new NotFoundException(`Header order not found (order_no=${dto.Order_no})`);
    }
  ...
  findAllHeaderOrder() {
    return this.headerOrderRepository.createQueryBuilder('headerOrder')
  ...
  findHeaderOrderByOrdDate(dto: FindHeaderOrderByDateDto) {
    ...
    return this.headerOrderRepository.createQueryBuilder('headerOrder')
      ...
      .where('detailOrder.ord_date BETWEEN :startDate AND :endDate', { startDate, endDate })
  ...
  findHeaderOrderByFinDate(dto: FindHeaderOrderByDateDto) {
    ...
    return this.headerOrderRepository.createQueryBuilder('headerOrder')
      ...
      .addSelect('detailOrder.tot_prc', 'tot_prc')
      .addSelect('headerOrder.order_date', 'order_date')
      .where('detailOrder.fin_date BETWEEN :startDate AND :endDate', { startDate, endDate })
      .getRawMany();
  }
  ...
  async createDetailOrder(dto: CreateDetailOrderDto[]) {
    ...
    for (const detail of dto) {
      const [headerOrder, goods] = await Promise.all([
        this.headerOrderRepository.findOne({ where: { order_no: detail.Order_no } }),
        this.goodsNameRepository.findOne({ where: { goods_id: detail.Goods_id } }),
      ]);

      if (!headerOrder) {
        throw new NotFoundException(`Header order not found (order_no=${detail.Order_no})`);
      }
      if (!goods) {
        throw new NotFoundException(`Goods not found (goods_id=${detail.Goods_id})`);
      }
  ...
  async deleteDetailOrder(dto: DeleteDetailOrderDto) {
    ...
    if (!detailOrder) {
      throw new NotFoundException(`Detail order not found (order_no=${dto.Order_no}, goods_id=${dto.Goods_id})`);
    }
  ...
  async processMasterOrder(dto: FindHeaderOrderByDateDto) {
    ...
    if (Array.from(headerOrderIdToDelete).length > 0) {
      const headerOrderToDelete = await this.headerOrderRepository.createQueryBuilder('headerOrder')
        .leftJoinAndSelect('headerOrder.d_order', 'detailOrder')
        .where('headerOrder.order_no IN (:...ids)', { ids: Array.from(headerOrderIdToDelete) })
        .getMany();
      await this.headerOrderRepository.remove(headerOrderToDelete.filter(hOrder => hOrder.d_order.length === 0));
    }
